...
const calcCosteo = (p) => {
  const costoPiedras   = (p.piedras || []).reduce((s, pp) => s + Number(pp.costoEstandardTotal), 0);
  const costoOro       = Number(p.gramosOro) * Number(p.costoGramoOroUsado);
  const costoTotal     = costoPiedras + costoOro + Number(p.costoManoObra) + Number(p.costoOtros);
  ...
  const margen         = precioVenta > 0 ? Math.round(((precioVenta - costoTotal) / precioVenta) * 10000) / 100 : 0;
  ...
  return { costoPiedras, costoOro, costoTotal, precioSugerido, pvpConIva, margen, ivaValor, conTarjeta, comisionMax };
};
  ...
  Mutation: {
    crearProducto: async (_, { input }, { prisma, user }) => {
      ...
      return prisma.producto.create({
        data: { ...input, multiplicador: input.multiplicador ?? 2.25, enStock: 0, activo: true, usu_creacion: user.codigo },
        include: incBom,
      });
    },
    ...
    actualizarProducto: async (_, { input }, { prisma, user }) => {
      ...
      const result = await prisma.producto.updateMany({
        ...
        data: { ...data, multiplicador: data.multiplicador ?? 2.25, version: { increment: 1 }, usu_actualizacion: user.codigo },
      });
    ...
    agregarInsumoProducto: async (_, { input }, { prisma, user }) => {
      ...
      const existe = await prisma.productoPiedra.findFirst({
        where: { productoId: input.productoId, tipoId: input.tipoId, deletedAt: null },
      });
      if (existe) throw new Error('Ya existe una piedra de este tipo en el producto');
      const costoEstandardTotal = Number(input.cantidad) * Number(input.costoEstandardUnitario);
      return prisma.productoPiedra.create({
        data: { ...input, costoEstandardTotal, desperdicio: input.desperdicio ?? 0 },
        include: { piedra: { include: { tipo: true, unidad: true } }, tipoPiedra: true },
      });
    },
    ...
    actualizarInsumoProducto: async (_, { input }, { prisma, user }) => {
      ...
      const result = await prisma.productoPiedra.updateMany({
        where: { id: Number(id), version: Number(version) },
        data: { ...data, costoEstandardTotal, desperdicio: data.desperdicio??0, version: { increment: 1 } },
      });
      if (result.count === 0) throw new Error('Modificado por otro usuario');
      return prisma.productoPiedra.findUnique({ where: { id: Number(id) }, include: { piedra: { include: { tipo: true, unidad: true } }, tipoPiedra: true } });
    },
