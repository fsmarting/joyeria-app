...
export const camposProducto = [
  {
    nombre: 'foto', etiqueta: 'Foto', tipoForm: 'text', maxLength: 500, ancho: '70px', ordenListado: 1,
  ...
  { nombre: 'costoGramoOroUsado', etiqueta: '$ / Gramo',    tipoForm: 'number', ancho: '110px', ordenListado: 6, valorDefecto: 0, render: (f) => fmt(f.costoGramoOroUsado) },
  ...
  { nombre: 'costoOtros',         etiqueta: 'Empaques',     tipoForm: 'number', ancho: '90px',  ordenListado: 8, valorDefecto: 0, render: (f) => fmt(f.costoOtros) },
  {
    nombre: 'multiplicador', etiqueta: 'Multiplicador', tipoForm: 'number', ancho: '90px', ordenListado: 9, valorDefecto: 2.25,
    render: (f) => `×${Number(f.multiplicador??2.25).toFixed(2)}`,
  },
  { nombre: 'costoTotal',     etiqueta: 'Costo Total',  soloListado: true, ancho: '110px', ordenListado: 10, render: (f) => fmt(f.costoTotal) },
  { nombre: 'precioSugerido', etiqueta: 'P. Sugerido', soloListado: true, ancho: '110px', ordenListado: 11, render: (f) => fmt(f.precioSugerido) },
  { nombre: 'pvpConIva',      etiqueta: 'PVP + IVA',   soloListado: true, ancho: '110px', ordenListado: 12, render: (f) => fmt(f.pvpConIva) },
  { nombre: 'precioVenta',    etiqueta: 'Precio Venta', tipoForm: 'number', ancho: '120px', ordenListado: 13, valorDefecto: 0, render: (f) => fmt(f.precioVenta) },
  {
    nombre: 'margen', etiqueta: 'Margen', soloListado: true, ancho: '80px', ordenListado: 14,
    render: (f) => {
  ...
  {
    nombre: 'enStock', etiqueta: 'Stock', soloListado: true, ancho: '80px', ordenListado: 15,
    render: (f) => {
