...
export const camposConversacion = [
  ...
  },
  {
    ...
    render: (f) =>
      f.producto ? (
        `${f.producto.referencia} — ${f.producto.nombre}`
      ) : (
  ...
  },
  {
    ...
    renderForm: ({ form, handleChange }) => (
      <select
        ...
        name="cotizo"
        value={String(form.cotizo ?? false)}
    ...
    render: (f) => (
      <span className={`badge ${f.cotizo ? "bg-primary" : "bg-secondary"}`}>
        {f.cotizo ? "Sí" : "No"}
      </span>
    ),
  },
  {
    ...
    render: (f) => (
      <span className={`badge ${f.cerro ? "bg-success" : "bg-secondary"}`}>
        {f.cerro ? "Sí" : "No"}
      </span>
    ),
  },
  {
    ...
    renderForm: ({ form, handleChange }) => (
      <select
        className="form-select"
        name="usoProtocolo"
        value={String(form.usoProtocolo ?? false)}
        onChange={handleChange}
      >
        <option value="false">No</option>
        <option value="true">Sí</option>
    ...
    render: (f) => (
      <span
        className={`badge ${f.usoProtocolo ? "bg-success" : "bg-secondary"}`}
      >
        {f.usoProtocolo ? "Sí" : "No"}
      </span>
    ),
  },
  {
    nombre: "motivoPerdidaId",
    etiqueta: "Motivo no cierre",
    tipoForm: "select",
    ancho: "140px",
    ordenListado: 9,
    relationConfig: {
      ...
      valueField: "id",
      displayField: "nombre",
      fixedVariables: { catalogoCodigo: "CRM", subcatalogoCodigo: "MOTI" },
    },
