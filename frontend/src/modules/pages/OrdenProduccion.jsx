...
function MovimientosHistorial({ movimientos }) {
  ...
  return (
    <table className="table table-sm mb-0" style={{fontSize:11}}>
      <thead><tr><th>Tipo</th><th>Lote</th><th>Cantidad</th><th>Valor</th><th>Fecha</th><th>Nota</th></tr></thead>
      <tbody>
        {movimientos.map(m => (
          <tr key={m.id}>
...
function SugerenciaRow({ bom, cantidadProgramada, onConfirmar }) {
  ...
  return (
    <tr>
      ...
      <td>{fmtQ(cantidadNecesaria, unidad)}</td>
      <td>
    ...
    <div className="p-3 bg-light border-top">
      ...
      {(orden.detalles||[]).length>0 && (
        <table className="table table-sm table-striped align-middle mb-3" style={{fontSize:11}}>
          <thead><tr><th>Insumo</th><th>Lote inicial</th><th>Cant/pieza</th><th>$ Unit.</th><th>$ Total</th><th>Enviado (total)</th><th>Devuelto</th><th>Merma</th><th>Conciliación</th><th></th></tr></thead>
          <tbody>{(orden.detalles||[]).map(d=><DetalleRow key={d.id} d={d} ordenCompleta={pendientes<=0} onRegistrarMovimiento={handleRegistrarMovimiento} onEliminar={handleEliminar}/>)}</tbody>
        </table>
      )}
      {(orden.detalles||[]).length>0 && (
