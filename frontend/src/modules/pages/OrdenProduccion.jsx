...
// ── CAMBIO (ronda 36) — se agregan ENTRADA_CONSUMO/SALIDA_CONSUMO, los
// dos movimientos que registrarEntregaOrden crea automáticamente al
// entregar producto terminado (Mecanismo 1) — nunca se eligen a mano
// desde MovimientoForm, a diferencia de ADICIONAL/DEVOLUCION.
const BADGE_MOVIMIENTO = {
  INICIAL: "bg-secondary",
  ADICIONAL: "bg-warning text-dark",
  DEVOLUCION: "bg-success",
  ENTRADA_CONSUMO: "bg-info text-dark",
  SALIDA_CONSUMO: "bg-light text-muted border",
};
const LABEL_MOVIMIENTO = {
  INICIAL: "Envío inicial",
  ADICIONAL: "Envío adicional",
  DEVOLUCION: "Devolución",
  ENTRADA_CONSUMO: "Consumido (producto entregado)",
  SALIDA_CONSUMO: "Consumo confirmado",
};
            ...
            <td>
              {m.numeroRemision && (
                <button
                  ...
                  title="Imprimir remisión de envío"
                >
                  🖨️
                </button>
              )}
            ...
            {lotes
              ...
              .map((l) => (
                <option key={l.id} value={l.id}>
                  {l.compra?.numero} · Disp:{" "}
                  {fmtQ(l.cantidadDisponible, unidad)}
                </option>
              ))}
          </select>
        </div>
    ...
    <>
      <tr>
        ...
        <td style={{ fontSize: 11 }}>
          {d.compraInsumo?.compra?.numero} ·{" "}
          {fmtF(d.compraInsumo?.compra?.fecha)}
        </td>
        ...
        {/* ── NUEVO (ronda 36) — cuánto de lo enviado ya se dio por
            consumido (convertido en producto terminado), actualizado
            automáticamente en cada entrega — ver Mecanismo 1. */}
        <td>
          {Number(d.cantidadConsumida) > 0 ? (
            <span
              className="badge bg-info text-dark"
              title={`Valor: ${fmt(d.valorConsumido)}`}
            >
              {fmtQ(d.cantidadConsumida, u)}
            </span>
          ) : (
            <span className="text-muted">—</span>
          )}
        ...
        {/* 🩹 antes decía "Merma" con badge amarillo de advertencia — este
            número es cantidadEnviada − cantidadDevuelta − cantidadConsumida
            (ronda 36), es decir "lo que el joyero todavía tiene en su
            poder sin convertir en producto terminado", NO desperdicio
            real. Mientras la orden está en curso es normal que sea alto —
            la alerta real de desperdicio ya la da la columna Conciliación
            de al lado. Se renombra y se quita el color de advertencia
            para no confundir "en poder del joyero" con "algo salió mal". */}
        <td>
          {Number(d.merma) > 0 ? (
            <span className="badge bg-secondary">{fmtQ(d.merma, u)}</span>
          ) : (
            <span className="text-muted">—</span>
          )}
        </td>
...
function SugerenciaRow({
  ...
  const stockAlcanza =
    !compraActual || cantidadAEnviar <= Number(compraActual.cantidadDisponible);
  const valido = !!compraSelId && cantidadAEnviar > 0 && stockAlcanza;

  useEffect(() => {
    onEstadoChange(bom.id, {
      valido,
      input: valido ? inputActual : null,
      stockAlcanza,
    });
    ...
    <tr>
      <td>
        <input
          ...
          title={
            valido
              ? "Incluir en el envío por lote"
              : "Elija un lote con stock suficiente para poder incluirlo"
          }
        ...
        {!compraSelId && (
          <div className="text-muted" style={{ fontSize: 9, maxWidth: 70 }}>
            Elija lote
          </div>
        )}
        ...
        >
          ...
          {lotes.map((c) => (
            <option key={c.id} value={c.id}>
              {c.compra?.numero} · {fmt(c.costoUnitario)}/{unidad} · Disp:{" "}
...
function DetallesPanel({ orden, refetch }) {
  ...
  const puedeCerrarParcial =
    Number(orden.cantidadEntregada) > 0 &&
    Number(orden.cantidadEntregada) < Number(orden.cantidadProgramada) &&
    estadoCodigo !== "CANC" &&
    estadoCodigo !== "ENTR";

  const handleCerrarOrden = async () => {
    if (!motivoCierre.trim())
      return toast.warning("Indique el motivo del cierre");
    if (
      !window.confirm(
        "¿Cerrar esta orden sin completar las piezas programadas? Esta acción no se puede deshacer.",
      )
    )
      return;
    try {
  ...
  const filasParaLote = [...seleccionadas].filter(
    (id) => estadoFilas[id]?.valido,
  );

  const handleConfirmarLote = async () => {
    if (filasParaLote.length === 0)
      return toast.warning(
        "Marque al menos un insumo (elija primero su lote y cantidad)",
      );
    try {
      await agregarLote({
        variables: {
      ...
      toast.success(
        `Envío registrado — ${filasParaLote.length} insumos bajo una sola remisión`,
      );
  ...
  const handleImprimirRemision = (m) => {
    ...
    for (const det of orden.detalles || []) {
      for (const mov of det.movimientos || []) {
        if (mov.numeroRemision && mov.numeroRemision === m.numeroRemision)
          lineas.push({ mov, det });
      }
    }
    ...
    if (!win) {
      toast.error(
        "El navegador bloqueó la ventana — permita ventanas emergentes para imprimir la remisión",
      );
      return;
    }
    win.document
      .write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>${m.numeroRemision || "Remisión de envío"}</title>
      <style>
    ...
    <div className="p-3 bg-light border-top">
      ...
      <div className="d-flex align-items-center gap-3 mb-3 flex-wrap">
        ...
        {puedeCerrarParcial && !cerrando && (
          <button
            className={`btn btn-outline-warning btn-sm ${puedeCancelar ? "" : "ms-auto"}`}
            onClick={() => setCerrando(true)}
          >
            🔒 Cerrar orden (entrega parcial)
          </button>
        )}
      </div>
      ...
      {cerrando && (
        <div
          className="border border-warning rounded p-2 bg-white mb-3"
          style={{ fontSize: 12 }}
        >
          <div className="fw-bold text-warning mb-2">
            Cerrar orden {orden.numero} con entrega parcial
          </div>
          <div
            className="alert alert-secondary py-1 px-2 mb-2"
            style={{ fontSize: 11 }}
          >
            Quedará con {orden.cantidadEntregada} de {orden.cantidadProgramada}{" "}
            piezas entregadas — la cantidad programada NO se modifica (queda
            como historia de para cuánto se envió material). Lo que pase con el
            insumo del faltante (devolución, reconocimiento de valor, o pérdida)
            se resuelve aparte con el joyero.
          </div>
          <div className="d-flex flex-wrap gap-2 align-items-end">
            <div style={{ flex: 1, minWidth: 240 }}>
              <label className="form-label mb-0">Motivo (obligatorio)</label>
          ...
          <tbody>
            {(orden.detalles || []).map((d) => (
              <DetalleRow
                ...
                onImprimirRemision={handleImprimirRemision}
              />
            ))}
          </tbody>
        </table>
        ...
        <div className="border rounded p-2 bg-white" style={{ fontSize: 12 }}>
          <div className="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-2">
            ...
            {filasParaLote.length > 0 && (
              ...
              >
                📦 Confirmar envío de {filasParaLote.length} seleccionados (1
                remisión)
              </button>
            )}
          </div>
          ...
          {sugerencias.length > 1 && (
            <div className="text-muted mt-2" style={{ fontSize: 10 }}>
              💡 Si va a entregar varios insumos juntos al joyero en un solo
              paquete, elija el lote de cada uno, marque sus checkboxes y use
              "Confirmar envío de seleccionados" — así todos quedan bajo una
              sola remisión imprimible en vez de una por insumo.
            </div>
          )}
        </div>
      )}
