...
import { Clientas, Joyeros, Proveedores, Socios } from './modules/pages/Tercero.jsx';
import { Empresas, Usuarios, UsuariosEmpresas, Catalogos, SubCatalogos, Grupos } from './modules/pages/Admin.jsx';
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      <button
        ...
        {icon} {label} <span style={{ fontSize: 9, opacity: .7 }}>▼</span>
      </button>
      {open && (
        <div style={{
          position: 'absolute', top: '100%', left: 0, zIndex: 1000,
          background: 'var(--bs-dark)', border: '1px solid rgba(255,255,255,0.15)',
          borderRadius: 8, minWidth: 180, boxShadow: '0 4px 16px rgba(0,0,0,.4)',
          padding: '4px 0',
        }}>
          {items.map((item, i) => (
            <Link
              ...
              style={{
                ...
                color: location.pathname === item.to ? '#ffc107' : 'rgba(255,255,255,.85)',
                textDecoration: 'none', fontWeight: location.pathname === item.to ? 500 : 400,
              }}
...
function Navbar({ usuario, empresa, onLogout }) {
  return (
    <nav className="navbar navbar-dark bg-dark px-3 py-0" style={{ minHeight: 48 }}>
      <Link className="navbar-brand fw-bold text-warning me-3" to="/" style={{ fontSize: 18 }}>💎</Link>
      <div className="d-flex align-items-center gap-1 flex-wrap flex-grow-1">

        {/* 📊 Análisis */}
        <DropdownMenu icon="📊" label="Análisis" items={[
          { to: '/',      label: 'Dashboard' },
          { to: '/metas', label: 'Metas mensuales' },
        ]}/>

        {/* 💰 Comercial */}
        <DropdownMenu icon="💰" label="Comercial" items={[
        ...
        {/* 🏭 Operaciones */}
        <DropdownMenu icon="🏭" label="Operaciones" items={[
        ...
        {/* 📦 Inventario */}
        <DropdownMenu icon="📦" label="Inventario" items={[
          { to: '/productos', label: 'Productos & BOM' },
        ]}/>

        {/* ⚙️ Admin */}
        <DropdownMenu icon="⚙️" label="Admin" items={[
      ...
      <div className="d-flex align-items-center gap-2 ms-2">
        <small className="text-white-50" style={{ fontSize: 11 }}>{usuario?.nombre} · {empresa?.nombre}</small>
        <button className="btn btn-outline-light btn-sm py-0" style={{ fontSize: 12 }} onClick={onLogout}>Salir</button>
      </div>
    </nav>
  );
}
...
function App() {
  ...
  useEffect(() => {
    const token=localStorage.getItem('token'), usuario=localStorage.getItem('usuario'),
          empresa=localStorage.getItem('empresa'), rol=localStorage.getItem('rol');
    if (token && usuario) setSesion({ token, usuario: JSON.parse(usuario), empresa: empresa?JSON.parse(empresa):null, rol: rol?JSON.parse(rol):null });
    setVerificando(false);
  }, []);
  const handleLogin  = ({ token, usuario, empresa, rol }) => setSesion({ token, usuario, empresa, rol });
  const handleLogout = () => { localStorage.clear(); setSesion(null); };

  if (verificando) return null;
  if (!sesion) return <><Login onLogin={handleLogin}/><ToastContainer position="top-right" autoClose={3000}/></>;

  return (
    <>
      <Navbar usuario={sesion.usuario} empresa={sesion.empresa} onLogout={handleLogout}/>
