...
export default function TablaDatos({
  ...
  const columnas = useMemo(
    () =>
      campos
        .filter((c) => !c.soloFormulario && c.ocultarEnListado !== true)
        .sort((a, b) => (a.ordenListado ?? 9999) - (b.ordenListado ?? 9999)),
    [campos],
  );

  const tieneColumnaAccionesCustom = useMemo(
    () => columnas.some((c) => c.nombre === "acciones"),
    [columnas],
  );
  const mostrarAcciones =
    (onEditar || onEliminar || accionesExtra) && !tieneColumnaAccionesCustom;

  const renderOrdenIcon = (nombreCol) => {
    const index = Array.isArray(ordenCampo)
      ? ordenCampo.indexOf(nombreCol)
      : -1;
    if (index === -1) return "↕";
  ...
  const colSpan =
    columnas.length + (mostrarAcciones ? 1 : 0) + (hayDetalle ? 1 : 0);
      ...
      <table className="table table-sm table-striped align-middle">
        ...
        <tbody>
          {estaCargando && (
            <tr>
              <td colSpan={colSpan}>
                <div className="py-3 text-center">Cargando datos…</div>
              </td>
            </tr>
          )}
                ...
                <div className="py-5 text-center">
                  {errorCarga.message === "Failed to fetch" ? (
                    <>
                      ...
                      <h5 className="text-secondary">Sin conexión</h5>
                    </>
                  ) : (
                    <>
                      <div style={{ fontSize: "2.5rem" }}>⚠️</div>
                      <h5 className="text-danger">Error del sistema</h5>
                    </>
                  )}
                </div>
              </td>
            ...
            <tr>
              <td colSpan={colSpan}>
                <div className="py-3 text-center text-muted">
                  {terminoBusqueda ? (
                    <>
                      No hay resultados para "<b>{terminoBusqueda}</b>".
                    </>
                  ) : (
                    "No hay registros."
                  )}
            ...
            registros.map((row) => {
              ...
              return (
                <Fragment key={rowKey}>
                  <tr>
                    ...
                    {columnas.map((col) => {
                      if (typeof col.render === "function") {
                        return (
                          <td key={col.nombre} className={col.className}>
                            {col.render(row)}
                          </td>
                        );
                      }
                  ...
                  {hayDetalle && abierto && (
                    <tr>
                      <td colSpan={colSpan}>{getDetalle(row)}</td>
                    </tr>
                  )}
                </Fragment>
              );
...
TablaDatos.propTypes = {
  ...
  onOrdenar: PropTypes.func,
  ordenCampo: PropTypes.array,
  ordenDireccion: PropTypes.array,
  onEditar: PropTypes.func,
  onEliminar: PropTypes.func,
  estaCargando: PropTypes.bool,
  terminoBusqueda: PropTypes.string,
  getDetalle: PropTypes.func,
  accionesExtra: PropTypes.func,
};
